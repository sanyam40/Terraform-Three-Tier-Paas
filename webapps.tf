...
resource "azurerm_linux_web_app" "frontend" {
  ...
  site_config {
    ...
    application_stack {
      ...
      docker_registry_url      = "https://${azurerm_container_registry.acr[count.index].login_server}"
      docker_registry_password = azurerm_container_registry.acr[count.index].admin_password
      docker_registry_username = azurerm_container_registry.acr[count.index].admin_username
    }
    ip_restriction {
      priority    = 500
...
resource "azurerm_linux_web_app" "backend" {
  count               = var.deploy_main_resource_group && var.deploy_backend_webapp ? 1 : 0
  name                = "${var.environment}-${var.app_name_backend}-${var.location_short}"
  location            = var.location
  resource_group_name = azurerm_resource_group.rg-main[count.index].name
